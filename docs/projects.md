# Projects

Some of my open-source projecs can be found in [GitHub](http://github.com/gergelyk/). Featured ones are listed below.

-----

## [digitout](https://gergelyk.github.io/python-digitout/)

**Tags**: `Python` `Development`

Seek data structures of your Python application.

<asciinema-player autoplay=true preload=true speed=2 font-size=9px src="https://raw.githubusercontent.com/gergelyk/python-digitout/master/docs/assets/demo.cast"></asciinema-player>

-----

## [kickoff](https://python-kickoff.readthedocs.io/en/latest/)

**Tags**: `Python` `Development`

Turns your Python script or module into an application with decent CLI.

![](https://python-kickoff.readthedocs.io/en/latest/_images/demo-app.png)

-----

## [peepshow](https://github.com/gergelyk/peepshow)

**Tags**: `Python` `Development`

Debugging utility for Python. Provides an interactive shell where you can browse data structures of your application.

![](https://user-images.githubusercontent.com/11185582/51219128-b3127780-192f-11e9-8618-ecfff642b87f.gif)

-----

## [bkp](https://github.com/gergelyk/bkp)

**Tags**: `Python` `Linux`

Simple but handy utility that can backup your files/directories and restore them later.

![](https://user-images.githubusercontent.com/11185582/48983793-ab82df00-f0f3-11e8-8727-c665b92bdb31.gif)

-----

## [repoeditor](https://github.com/gergelyk/repoeditor)

**Tags**: `Python` `Windows` | **Status**: Not maintained

Editor of small and medium size repositories for [Npackd](https://npackd.appspot.com/) package manager.

-----

## [git-rsync](https://github.com/gergelyk/git-rsync)

**Tags**: `Nimrod` `git`

Free solution for adding large file support to your git repository.


